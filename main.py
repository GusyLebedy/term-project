...
def handler(event, context):
    ...
    if event['state']['session'] == {}:
        ...
        elif command_.lower() == "оставить":
            object_, text = next_card1(object_)
        elif command_.lower() == "забрать":
            object_ = take_card(object_)
            object_, text = next_card1(object_)
        else:
            text = 'Неверное значение, попробуй еще раз.'
...
class Game():
    player_counter = 0
    win_count = random.randint(80, 104)
    current_card_streak = []
    position = 0
    card_shuffle = []
...
def start():
    ...
    if count == 0:
        text = 'Привет, капитан' + '\n' + 'Здесь, в таверне «Адмирал Бенбоу», мы собрались, чтобы поделить сокровища.' + \
               ...
               ' Ты можешь закончить рейд грабежом и получить очки за этот трофей, а можешь испытать удачу и продолжить тянуть карты, чтобы заработать больше очков.' + \
               ...
               ' даже если рейд будет провален.' + ' Играй с умом и помни, жадность – это плохо.' + '\n' + '\n' + 'Чтобы прервать рейд скажи "забрать", чтобы продолжить - "оставить".' + '\n' + '\n' + 'Количество очков для победы: ' + str(
            new_object.win_count) + '\n'
        count += 1
    else:
